...
-- Define a custom type for the JSON structure of vagas to improve readability and type safety
-- This step is optional but good practice. For simplicity here, we'll cast directly from jsonb.
...
RETURNS uuid -- Retorna o ID da reserva criada
LANGUAGE plpgsql
SECURITY DEFINER -- Executa com os privilégios do usuário que definiu a função
AS $$
DECLARE
    v_reserva_id uuid;
...
BEGIN
    ...
    LOOP
        ...
        IF EXISTS (
            ...
            WHERE rv.vaga_id = v_vaga_id_check
              AND r.status NOT IN ('cancelada', 'rejeitada')
    ...
    INSERT INTO public.reservas (
        ...
        'pendente' -- Status inicial da reserva
    ...
    LOOP
        INSERT INTO public.reserva_vagas (
            ...
            data_entrada,
            data_saida,
            valor_diaria, -- Assumindo que 'preco' no JSON é o valor total para o período
            preco_aplicado
        ) VALUES (
            v_reserva_id,
            ...
            (vaga_item ->> 'preco')::numeric, -- Ajuste se o significado do preço for diferente
...
EXCEPTION
    WHEN OTHERS THEN
        -- Em caso de qualquer erro, loga e relança a exceção
        RAISE NOTICE 'Ocorreu um erro ao criar a reserva: %', SQLERRM;
        RAISE;
END;
$$;

-- Function to atomically cancel a reservation
